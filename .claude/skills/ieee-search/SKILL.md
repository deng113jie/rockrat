...
Where `{QUERY}` is the URL-encoded search keywords from `$ARGUMENTS`. When the `$ARGUMENTS` is more than three words long, it is very likely to be a paper title rather than keyword. In this case, remove the special characters from the `$ARGUMENTS` and encoding the full text in HTML, to use as `{QUERY}` 
